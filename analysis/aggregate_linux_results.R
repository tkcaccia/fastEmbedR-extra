...
results_root <- if (length(args) >= 1L) args[[1L]] else stop(
  "Usage: aggregate_linux_results.R RAW_RESULTS_ROOT OUTPUT_DIR",
  call. = FALSE
)
output_dir <- if (length(args) >= 2L) args[[2L]] else stop(
  "An aggregate output directory is required.", call. = FALSE
)
...
method_labels <- c(
  fastEmbedR_tsne_cpu_full = "fastEmbedR t-SNE CPU",
  fastEmbedR_tsne_cuda_full = "fastEmbedR t-SNE CUDA",
  Rtsne_full = "Rtsne",
...
comparisons <- rbind(
  matched_speedup("fastEmbedR_tsne_cpu_full", "Rtsne_full", "t-SNE"),
  matched_speedup("fastEmbedR_tsne_cuda_full", "Rtsne_full", "t-SNE"),
...
key_methods <- c(
  "fastEmbedR_tsne_cpu_full", "fastEmbedR_tsne_cuda_full",
...
method_short <- c(
  "fastEmbedR t-SNE CPU" = "fastEmbedR t-SNE",
  "fastEmbedR t-SNE CUDA" = "fastEmbedR t-SNE",
  "Rtsne" = "Rtsne",
  "FIt-SNE" = "FIt-SNE",
...
tsne_methods <- c(
  "fastEmbedR t-SNE CPU", "fastEmbedR t-SNE CUDA", "Rtsne", "FIt-SNE"
)
umap_methods <- c(

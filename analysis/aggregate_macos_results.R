...
method_mode <- function(method) {
  if (grepl("tsne", method)) return("t-SNE")
...
full <- successful[
  successful$timing_scope == "full_pipeline" &
    grepl("tsne|umap", successful$method) &
...
draw_runtime_quality <- function(device) {
  ...
  draw_metric_panel(full, "t-SNE", "total_runtime_sec_median", "log10 total seconds", TRUE)
  draw_metric_panel(full, "UMAP fuzzy", "total_runtime_sec_median", "log10 total seconds", TRUE)
  draw_metric_panel(full, "t-SNE", "trustworthiness_median", "Trustworthiness")

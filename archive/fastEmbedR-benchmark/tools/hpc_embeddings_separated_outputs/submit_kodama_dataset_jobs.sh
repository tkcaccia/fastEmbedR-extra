...
# Submit only the slow KODAMA workflows. Each dataset/backend launcher fits
# KODAMA KNN and PLS-LDA once per seed, then reuses each fit for openTSNE and
# UMAP visualization.
# Examples:
...
case "${PROFILE}" in
  all)   scripts=("${JOBS_DIR}"/run_*_kodama_*.sh) ;;
  cpu1)  scripts=("${JOBS_DIR}"/run_*_kodama_cpu1.sh) ;;
  cpu4)  scripts=("${JOBS_DIR}"/run_*_kodama_cpu4.sh) ;;
  cuda)  scripts=("${JOBS_DIR}"/run_*_kodama_cuda.sh) ;;
  *)
...
if [[ ! -d "${JOBS_DIR}" || ! -f "${scripts[0]}" ]]; then
  echo "No KODAMA ${PROFILE} launchers found under ${JOBS_DIR}" >&2
  exit 1
fi
...
echo "Submitted KODAMA jobs: ${submitted}"

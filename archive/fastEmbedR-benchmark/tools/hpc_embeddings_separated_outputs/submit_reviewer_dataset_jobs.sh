...
case "${PROFILE}" in
  all)   scripts=("${JOBS_DIR}"/run_*_standard_*.sh) ;;
  cpu1)  scripts=("${JOBS_DIR}"/run_*_standard_cpu1.sh) ;;
  cpu4)  scripts=("${JOBS_DIR}"/run_*_standard_cpu4.sh) ;;
  cuda)  scripts=("${JOBS_DIR}"/run_*_standard_cuda.sh) ;;
  *)

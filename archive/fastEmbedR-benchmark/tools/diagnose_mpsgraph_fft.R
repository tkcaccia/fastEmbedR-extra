...
rows <- lapply(sizes, function(size) {
  message("Testing MPSGraph FFT round-trip size ", size)
  out <- fastEmbedR:::metal_mpsgraph_fft_diagnostic_cpp(
    fft_size = size,
    seed = seed,
    n_repeats = n_repeats
  )
  data.frame(
    ...
    median_time_sec = as.numeric(out$median_time_sec %||% NA_real_),
    first_time_sec = as.numeric(out$first_time_sec %||% NA_real_),
...
csv <- file.path(out_dir, paste0("mpsgraph_fft_diagnostic_", stamp, ".csv"))
write.csv(res, csv, row.names = FALSE)
write.csv(res, file.path(out_dir, "latest_mpsgraph_fft_diagnostic.csv"), row.names = FALSE)
print(res)
message("Wrote ", csv)

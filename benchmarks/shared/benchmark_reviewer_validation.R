...
parameter_record <- function(method, requested_threads) {
  ...
  data.frame(
    ...
    iterations_or_epochs = if (family == "t-SNE") {
      ...
      } else if (grepl("Rtsne|FItSNE", method)) "750 iterations" else "fastEmbedR t-SNE auto policy (250 early + 500 normal default)"
    } else if (family == "UMAP") {
      if (method_is_kodama(method)) {
    ...
    early_exaggeration = if (family == "t-SNE") {
      ...
      } else if (grepl("Rtsne", method)) "12 for first 250 iterations" else "fastEmbedR t-SNE auto policy"
    } else "not applicable",

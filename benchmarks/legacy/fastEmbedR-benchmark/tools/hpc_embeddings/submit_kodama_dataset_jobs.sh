...
# Submit the independent KODAMA-only jobs. Each dataset/profile has its own
# Slurm job, so a failure or timeout does not prevent the remaining datasets.
#
# Examples:
#   bash submit_kodama_dataset_jobs.sh
#   PROFILE=cpu4 bash submit_kodama_dataset_jobs.sh
#   PROFILE=cuda DRY_RUN=true bash submit_kodama_dataset_jobs.sh

set -uo pipefail

BASE_DIR="${BASE_DIR:-/scratch/firenze/NN}"
JOBS_DIR="${JOBS_DIR:-${BASE_DIR}/kodama_jobs_by_dataset}"
PROFILE="${PROFILE:-all}"
DRY_RUN="${DRY_RUN:-false}"
SUBMIT_SLEEP="${SUBMIT_SLEEP:-0.1}"
LOG_DIR="${LOG_DIR:-${BASE_DIR}/benchmark_logs}"
STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
SUBMISSION_LOG="${SUBMISSION_LOG:-${LOG_DIR}/kodama_job_submissions_${PROFILE}_${STAMP}.csv}"

case "${PROFILE}" in
  all)  scripts=("${JOBS_DIR}"/run_kodama_*.sh) ;;
  cpu1) scripts=("${JOBS_DIR}"/run_kodama_*_cpu1.sh) ;;
  cpu4) scripts=("${JOBS_DIR}"/run_kodama_*_cpu4.sh) ;;
  cuda) scripts=("${JOBS_DIR}"/run_kodama_*_cuda.sh) ;;
  *) echo "PROFILE must be one of: all, cpu1, cpu4, cuda" >&2; exit 2 ;;
esac

if [[ ! -d "${JOBS_DIR}" || ! -f "${scripts[0]}" ]]; then
  echo "No ${PROFILE} KODAMA launchers found under ${JOBS_DIR}" >&2
  exit 1
fi
